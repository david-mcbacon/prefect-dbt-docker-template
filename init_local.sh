...
function start_server() {
    ...
    sleep 1
    docker-compose build prefect-server # if I make changes in prefect/Dockerfile, rebuild image
...
function initialize() {
    ...
    sleep 1
    set +e
    docker-compose exec prefect-server bash -c 'cd /app/prefect/flows && python ./init_orion.py'
    if [ $? -ne 0 ]; then
